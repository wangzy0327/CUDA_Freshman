...
*/
//M=N=K=6144:  GPU mySgemm Average elasped time: 0.037106 second, performance: 12500.853114 GFLOPS.
__global__  __launch_bounds__(256)
void mysgemm_v11_ano(int M, int N, int K, float alpha, const float* A, const float* B, float beta, float* C){
    ...
    __shared__ float sa11[2*KS_11*MS_11];
    __shared__ float sb11[2*KS_11*NS_11];
    float *ptr_sa11, *ptr_sb11;
    ptr_sa11 = (float*)sa11;
    ptr_sb11 = (float*)sb11;
    int row_a = (tx%32)<<2, col_a = tx/32; //row_a [0,4,8,...,120,124] col_a [0,1,2,...,7]
    int col_b = tx%NS_11, row_b = (tx>>7)<<2; //col_b [0-127] row_b [0,4]
    ...
    for(int k_count = 0;k_count < K_upper;k_count++){
        ...
        // __syncthreads();
        #pragma unroll
        for(int inner_k = 0;inner_k < KS_11;inner_k++){
            int next_inner_k = (inner_k + 1)%KS_11;
            //加载 double buffer 的另一个
            //计算inner_k 加载inner_k+1
            //编译器/手写循环的依赖
            //如果 vload 和 vscal 在同一个 inner_k 循环里并且顺序写死，那么编译器会保持顺序执行，这样不会提前进入下一轮
            if(next_inner_k != 0){
                vload(Av1[(inner_k+1)&1],&ptr_sa11(row_c,next_inner_k));
                vload(Av2[(inner_k+1)&1],&ptr_sa11(row_c+4,next_inner_k));
                vload(Bv1[(inner_k+1)&1],&ptr_sb11(col_c,next_inner_k));
                vload(Bv2[(inner_k+1)&1],&ptr_sb11(col_c+4,next_inner_k));
            }
        ...
        // __syncthreads();
        // ((float4 *)sa11)[tx] = pref_Av;
        // sb11(col_b,row_b)=pref_Bv.x;
        // sb11(col_b,row_b+1)=pref_Bv.y;
        // sb11(col_b,row_b+2)=pref_Bv.z;
        // sb11(col_b,row_b+3)=pref_Bv.w;
        ...
        ptr_sb11(col_b,row_b+3)=pref_Bv.w;
        __syncthreads();
        // vload(Av1[0], &sa11(row_c,0));
        // vload(Av2[0], &sa11(row_c+4,0));
        // vload(Bv1[0], &sb11(col_c,0));
        // vload(Bv2[0], &sb11(col_c+4,0));     
        vload(Av1[0], &ptr_sa11(row_c,0))
        vload(Av2[0], &ptr_sa11(row_c+4,0))
        vload(Bv1[0], &ptr_sb11(col_c,0))
        vload(Bv2[0], &ptr_sb11(col_c+4,0))           
