...
{
    int value=in[threadIdx.x];
    value=__shfl_down_sync(value,delta,BDIM);
    out[threadIdx.x]=value;
...
{
    int value=in[threadIdx.x];
    value=__shfl_sync(value,threadIdx.x+offset,BDIM);
    out[threadIdx.x]=value;
...
{
    int value=in[threadIdx.x];
    value=__shfl_xor_sync(value,mask,BDIM);
    out[threadIdx.x]=value;
...
__global__ void test_shfl_xor_array(int *in,int*out,int const mask)
{
    ...
    for(int i=0;i<SEGM;i++)
        value[i]=in[idx+i];
    value[0]=__shfl_xor_sync(value[0],mask,BDIM);
    value[1]=__shfl_xor_sync(value[1],mask,BDIM);
    value[2]=__shfl_xor_sync(value[2],mask,BDIM);
    value[3]=__shfl_xor_sync(value[3],mask,BDIM);
    for(int i=0;i<SEGM;i++)
        out[idx+i]=value[i];
...
void swap(int *value,int laneIdx,int mask,int firstIdx,int secondIdx)
{
    ...
    value[secondIdx]=__shfl_xor_sync(value[secondIdx],mask,BDIM);
    if(pred)
    {
        int tmp=value[firstIdx];
        value[firstIdx]=value[secondIdx];
